...
Before discussing token management, ensure you have TLS certificates set up for secure communication with Vault. You can use self-signed certificates for development or obtain certificates from a trusted CA for production.

You can generate TLS certificates using the provided script `generate-tls.sh`. This script creates a Certificate Authority (CA) and signs a Vault server certificate with appropriate Subject Alternative Names (SANs) for secure communication.

Run `./generate-tls.sh` to create the certificates, which will be stored in the `tls/` directory.
...
### To update or insert all backend .env files for VAULT_ADDR, VAULT_TOKEN, VAULT_CACERT, VAULT_TRANSIT_KEY:

Run `bash ./update-service-env.sh `
